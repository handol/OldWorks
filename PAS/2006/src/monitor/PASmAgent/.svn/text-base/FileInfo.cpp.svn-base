...
#define ME_DIR_INFO "/export/home/sms4/pasgw/me/"
#define KUN_DIR_INFO "/export/home/sms4/pasgw/kun/"
...
CFileInfo::GetFileInfo(const char* szFileName, int iType,  char* szCreateDate, int &iFileSize, int &iStatus)
{
        int iRet = 0;
	...
	if ( iType == ME_TYPE )
	{
        	dir = opendir(ME_DIR_INFO);
	}
	else
	{
		dir = opendir(KUN_DIR_INFO);
	}
	...
	if ( dir == NULL )
	{
...
#endif		
		iStatus = -1;
		sprintf(m_szGetLastError, "%s-%s-%d", szFileName, "DIR_OPEN_ERROR", errno);
		return iRet;
	}
        ...
        while((pdirent = readdir(dir)) != NULL)
        {
			...
			if ( iType == ME_TYPE )
			{
                        	sprintf(filepath, "%s%s", ME_DIR_INFO, pdirent->d_name);
			}
			else
			{
				sprintf(filepath, "%s%s", KUN_DIR_INFO, pdirent->d_name);
			}
