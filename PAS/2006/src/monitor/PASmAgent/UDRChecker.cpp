...
int CUDRChecker::GetUDRFileList(const char* szPath, const char* szGWInfo, char Type, const char* szCurrentDate, char* szFileName)  
{
#ifdef DEBUG
	cout << szPath << endl;
#endif
	...
	char iRet = 0;
 	...
 	dir = opendir(szPath);
		...
		{
			if ( !strncmp(pdirent->d_name+25, szString2, strlen(szString2)) )
			{
				iRet++;
...
int main()
{
	...
	CUDRChecker udr;
	udr.SetFileName("/nfs2/paslog/N_SWPAS0_FWEBIDR1_ID0086_T20040914141000.DAT");
