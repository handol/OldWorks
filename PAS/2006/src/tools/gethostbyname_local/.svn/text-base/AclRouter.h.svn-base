...
public:
	...
	int init(const char *pHostID, int nPasgwPort);
	void remove_worker_thread(void);
	int load(const char *pFileName);
	int	searchDNS(const char *orgHost, const int orgPort, char *destHost, int &destPort);
	int	searchALL(const char *orgHost, const int orgPort, char *destHost, int &destPort);

	static AclRouter *instance();
	void getHost(char *host, int &port, const char *hostname);

	void test();

	#ifdef TEST_MAIN_GETHOSTBYNAME_LOCAL
	int dnsSocket;
	bool gethostbyname_local(const char *orgHost, char *destHost, int size);
	#endif
};
