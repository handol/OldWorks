	...
	while(run)
	{
		...
		switch( result )
		{
			case ACL_SYNTAX_ERROR :
				PAS_ERROR1( "%s 파일 구문검사 오류\n", aclFileName.toStr() );
				break;

			case ACL_FILE_NOT_FOUND :
				PAS_ERROR1( "%s File not found\n", aclFileName.toStr() );
				break;

			case ACL_VERSION_ERROR :
				PAS_WARNING1( "%s 파일의 버전이 이전 버전과 동일하여 리로드 하지 않습니다.", aclFileName.toStr() );
				break;

			case ACL_TAIL_ERROR :
				PAS_ERROR( "ACL 파일의 끝을 나타내는 ACLX 가 없습니다.\n" );
				break;

			default :
				PAS_NOTICE( "ACL Load Complete" );
				break;
		}
	}
...
*/
int AclRouter::init(const char *pHostID, int nPasgwPort)
{
	...
	{
		PAS_ERROR1("Acl파일[%s]가 없음", aclFileName.toStr());
		printf("Acl파일[%s]가 없음", aclFileName.toStr());
		return -1;
	}
...
bool AclRouter::setHost(char *pBuff)
{
	...
	strncpy( host->Port, hostSpliter.fldVal(nFldPos++), sizeof(host->Port) );
	strncpy( host->Opt, hostSpliter.fldVal(nFldPos++), sizeof(host->Opt) );
...
bool AclRouter::setDNS(char *pBuff)
{
	...
	strncpy( aclData->SrcPort, dnsSpliter.fldVal(nFldPos++), sizeof(aclData->SrcPort) );
	strncpy( aclData->DestAddr, dnsSpliter.fldVal(nFldPos++), DNS_URL_SIZE );
	strncpy( aclData->DestPort, dnsSpliter.fldVal(nFldPos++), sizeof(aclData->DestPort) );
	strncpy( aclData->Owner, dnsSpliter.fldVal(nFldPos++), sizeof(aclData->Owner) );
	strncpy( &aclData->OnOff, dnsSpliter.fldVal(nFldPos++), sizeof(aclData->OnOff) );
...
bool AclRouter::setACL(char *pBuff)
{
	...
	strncpy( aclData->SrcPort, ACLSpliter.fldVal(nFldPos++), sizeof(aclData->SrcPort) );
	strncpy( aclData->DestAddr, ACLSpliter.fldVal(nFldPos++), DNS_URL_SIZE );
	strncpy( aclData->DestPort, ACLSpliter.fldVal(nFldPos++), sizeof(aclData->DestPort) );
	strncpy( aclData->Owner, ACLSpliter.fldVal(nFldPos++), sizeof(aclData->Owner) );
...
{
	// 구문 검사
	if( syntaxCheck(pFileName) == false )
...
*/
int	AclRouter::searchDNS(const char *orgHost, const int orgPort, char *destHost, int &destPort)
{
...
*/
	strcpy( destHost, pData->DestAddr );
...
@return ACL_NOT_FOUND	url 길이가 너무 길거나 찾을 수 없다.
        ...
        ACL_FIND_DNS	DNS 목록에서 url 에 대한 데이터 발견
*/
int	AclRouter::searchALL(const char *orgHost, const int orgPort, char *destHost, int &destPort)
{
	if( strlen(orgHost) > 63+4 )
		return ACL_NOT_FOUND;
	...
	if( iterACL == mapACL.end() )
	{
		...
		if( iterDNS == mapDNS.end() )
		{
			...
			if( (int) inet_addr(orgHost) == -1 )
			{
				...
				lphost = gethostbyname( orgHost );
				if( lphost )
				{
					struct in_addr addr;
					memcpy( &addr, lphost->h_addr, 4 );
					key.SrcAddr = inet_ntoa(addr);
					key.SrcPort = orgPort;
					// ACL 목록에서 찾는다.
					iterACL = mapACL.find( key );
				}

				#elif TEST_MAIN_GETHOSTBYNAME 
				char destIP[16] = "\0";
				if( Util2::getHostByName_threadsafe(orgHost, destIP, sizeof(destIP)) )

				#elif TEST_MAIN_GETHOSTBYNAME_LOCAL
				char destIP[16] = "\0";
				if( gethostbyname_local(orgHost, destIP, sizeof(destIP)) )
...
*/
void AclRouter::getHost(char *host, int &port, const char *hostname)
{
...
#ifdef TEST_MAIN_GETHOSTBYNAME_LOCAL
bool AclRouter::gethostbyname_local(const char *orgHost, char *destHost, int size)
{
	if( dnsSocket == -1 )
	{
		dnsSocket = socket( AF_INET, SOCK_STREAM, 0 );
		struct sockaddr_in saddr;
		
		memset( &saddr, 0x00, sizeof(sockaddr_in) );
		saddr.sin_family = AF_INET;
		saddr.sin_addr.s_addr = inet_addr( Config::instance()->acl.dnshost );
		saddr.sin_port = htons( Config::instance()->acl.dnsport );
		
		// 타임아웃 시간 재설정
		linger lingtime;
		lingtime.l_onoff = 1;
		lingtime.l_linger = 3;
		setsockopt(dnsSocket, SOL_SOCKET, SO_LINGER, (void*)&lingtime, sizeof(linger));
		if( connect(dnsSocket, (struct sockaddr*)&saddr, sizeof(sockaddr_in)) < 0 )
		{
			if( errno != EWOULDBLOCK )
			{
	...
	int nWrite = send( dnsSocket, orgHost, strlen(orgHost), 0 );
	if( nWrite <= 0 )
	{
		close(dnsSocket);
//		printf("Connection closed from DNS\n");
		dnsSocket = -1;
		return false;
	}
...
//	printf("Send[%d] [%s]\n", nWrite, orgHost);
	...
	char recvbuff[256] = "\0";
	int nRead = recv( dnsSocket, recvbuff, sizeof(recvbuff), 0 );
	if( nRead <= 0 )
	{
		close(dnsSocket);
//		printf("Connection closed from DNS\n");
...
//	printf("Recv[%d] [%s]\n", nRead, recvbuff);
	...
	if( nRead < size )
		memcpy( destHost, recvbuff, nRead );
...
int main( int argc, char* argv[] )
{
	if( argc <= 1 )
	{
		printf("Usage : Aclrouter.test [Config.cfg] [URL]\n");
		return 0;
	}
...
*/
		if( pAcl->gethostbyname_local(argv[2], desturl, sizeof(desturl)) == false )
		{
			sleep(1);
			continue;
		}
		...
		else
		{
			...
			printf( "Delayed sec[%d] usec[%f] Result[%s]\n", delayedsec, delayedusec, desturl );
		}
