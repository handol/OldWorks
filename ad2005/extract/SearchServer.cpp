...
void	init_env()
{
	...
	sprintf(fname, "log/search-%s.txt", TimeUtil::get_yymmdd() );
	...
	if (adhome) ACE_DEBUG ((LM_NOTICE,	"ADHOME = %s\n", adhome));
	else ACE_DEBUG ((LM_INFO,	"ADHOME is NOT set !\n"));
	
	config.open_home(adhome, "anydict.cfg");
	PORT = config.GetIntVal("SEARCH_PORT", PORT);
...
int main (int, char **)
{
	...
	//StrUtil::path_merge(exdatapath, adhome, "data");
