...
int is_belong_to_verb_incomp_dic(int index)
{
	...
	len=strlen((char*)str);
							  // xxÇÏ °¡ ¾î°£ÀÎ °æ¿ì ÀÏ´Ü Å¸µ¿»ç·Î
	if( !strncmp((char*)str+len-2,"ÇÏ",2)) return 1;
	...
	while( p_list_result!=NULL)
	{
			...
			)
		{
			if( is_belong_to_aux_noun_type((char*)p_list_result->yongon.eomi,type) )
			{
				...
				if(index>0)	  // ÃÖ¼ÒÇÑ µÎ¹øÂ° ¾îÀı ÀÌÈÄ
...
{
	// ºĞ¼®¼º°øÇÏ¸é 0 ÀÌ ¾Æ´Ñ °ªÀ» ¹İÈ¯ , ÀÇÁ¸¸í»ç´Â À½¼ö ÀÏ¹İÃ¼¾ğÀº ¾ç¼ö
	L_RESULT_MA *p_list_result;
	unsigned char *p_str;
	int index_start;
	p_list_result=result_ma[index].p_list_result;
...
int check_chaon_possesive(int index)
{
	...
	while( p_list_result!=NULL)
	{
		if( p_list_result->type==CHAON_JOSA)
		{
			if( !strcmp((char*)p_list_result->chaon.josa,"ÀÇ") )
				return 1;
		}
		p_list_result=p_list_result->next;
	}
...
int analyze_modification(int index,int kind,int depth)
{
	...
	else if( type_of_next == A_CHAON || type_of_next == A_CHAON_YI)
	{
		...
		else if( check_yongon(index) )
		{
			...
			{
				analyze_yongon(index,depth+1);
			}				  // ¿ë¾ğÀº °üÇüÇüÀÌ ¾Æ´Ñ°æ¿ì ÃµÀ½Àı¿¡ ¿Ã ¼ö ¾ø´Ù.
			else if( index> 0)
			{
				analyze_yongon(index,depth+1);
			}
...
int analyze_yongon(int i,int depth)
{
	...
	{
		// ¹®ÀåÀÇ Ã¹ ¾îÀı¿¡´Â ¿°üÇüÇü ¿Ü¿¡´Â ¿ë¾ğÀÌ ¿ÀÁö ¸øÇÔ
		return 0;
	}						  // ±×·¯³ª ¿¿ë¾ğÀ¸·Î¸¸ ±¸¼ºµÈ ¹«ÀåÀº °¡´É
	else if(depth==0)
	{
...
// ºĞ¼®µÇÁö ¾ÊÀº °æ¿ì
// ¿¹¸¦ µé¾î ¸ñÀû¾î°¡ ¾ø´Â °æ¿ì ¸ñÀû¾î¿Í ¼ú¾î»çÀÌ ºĞ¼®
// ÁÖ¾î°¡ ¾ø´Â °æ¿ì ÁÖ¾î¿Í ¸ñÀû¾î »çÀÌ ºĞ¼®
// ÁÖ¾î ¾ÕÀÇ ºĞ¼®ÀÌ ¿ÏÀüÇÏÁö ¾ÊÀº °æ¿ì µîµî..
