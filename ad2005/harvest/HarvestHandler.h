...
class HarvestHandler : public ACE_Svc_Handler <ACE_SOCK_STREAM, ACE_NULL_SYNCH>
{
	public:
	...
	private:
		...
		*/
		...
		int harvest(char *expr_str, int early_flag=0);
		int sendRes();
		int webfetch(char *url);
		
		StrStream resStr;
		int	debug;
};

typedef ACE_Acceptor <HarvestHandler, ACE_SOCK_ACCEPTOR> HarvestAcceptor;
